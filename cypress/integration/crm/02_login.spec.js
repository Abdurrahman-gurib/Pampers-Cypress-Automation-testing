...
(getCrmMarketsScope(crmProperties.login)).forEach((loginScopeElement) => {
    describe(`Login for ${loginScopeElement} on ${crmProperties.env}`, () => {
        ...
        it(`type email address`, () => {
            cy.task("systemLog", `type email address ${crmProperties.login.properties[loginScopeElement].login}`);
            cy.get(crmSelector.login.email, {timeout: 5000}).type(crmProperties.login.properties[loginScopeElement].login, {force: true});
        });
        it(`type password`, () => {
            cy.task("systemLog", `type password ${crmProperties.login.properties[loginScopeElement].password}`);

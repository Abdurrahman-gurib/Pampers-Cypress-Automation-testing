...
(getMarketsScope(babyToolsProperties, null)).forEach(scopeElement => {
    ...
    if (babyToolsProperties.markets[scopeElement].vortexUrl) {
        const listOfVortexRegUrl = babyToolsProperties.markets[scopeElement].vortexUrl;
        const crmFieldsList = require('../../fixtures/testProperties/crm.properties.json')[`registration`][`properties`][scopeElement][`fields`];
        const crmSelectors = require('../../fixtures/cssSelector/pampersCrm.selectors.json')[`registration`];
        const fieldsToCheck = [`firstName`, `email`, `password`, `reactCalendarIcon`, ...crmFieldsList];
        listOfVortexRegUrl.forEach(vortexUrlElement => {
            const tempVortexUrl = `${tempMarketBaseUrl}/${vortexUrlElement.includes(`!`) ? vortexUrlElement.split(`!`)[0] : vortexUrlElement}`;
            const dontRun = (vortexUrlElement.split(`!`)[1])?.split(`,`);
            describe(`[Pampers ${scopeElement}][Vortex REG check form]`, () => {
                it(`[open vortex url][${tempVortexUrl}]`, () => {
                    cy.task(`systemLog`, `open vortex scenario ${tempVortexUrl}`);
                    cy.visit(encodeURI(tempVortexUrl), {timeout: 60000});
                });
                fieldsToCheck.forEach(fieldElement => {
                    if (!dontRun || !dontRun.find(dontRunElement => dontRunElement === fieldElement) && fieldElement !==`noFirstName`) {
                        it(`[verify vortex form field][${fieldElement}][${tempVortexUrl}]`, () => {
                            const tempFieldSelectorToCheck = (fieldElement === `gdpr`) ? crmSelectors.regAreYouParentSection : crmSelectors[fieldElement];
                            cy.task(`systemLog`, `check ${fieldElement} with selector ${tempFieldSelectorToCheck}`);
                            cy.get(tempFieldSelectorToCheck.includes(`|`) ? tempFieldSelectorToCheck.split(`|`)[0] : tempFieldSelectorToCheck).should("be.visible");
                        });
                    }
                });
            });
